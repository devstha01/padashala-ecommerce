...
<div class="content">
    <!-- BEGIN LOGIN FORM -->
    <form action="{{route('merchant-verify-post')}}" method="post">
        {{csrf_field()}}

        <h3 class="form-title">{{__('dashboard.Email Verification')}}</h3>
        <div class="alert alert-danger display-hide">
            <button class="close" data-close="alert"></button>
            <span>{{__('dashboard.Enter Email')}}</span>
        </div>
        @include('fragments.message')
        <div class="form-group">
            <label class="control-label visible-ie8 visible-ie9">{{__('dashboard.Email')}}</label>
